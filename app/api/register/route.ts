Set-Content "app\api\register\route.ts" @'
...
export async function POST(req: Request) {
  try {
    ...
    const { email, password, role, full_name, telephone, ville, metier, carte_identite_url, casier_judiciaire_url } = body;
    ...
    // 3. Insérer dans prestataires
    if (role === "prestataire" || role === "artisan") {
      const { error: prestaError } = await supabaseAdmin.from("prestataires").insert({
        user_id: userId,
        nom: full_name,
    ...
    await resend.emails.send({
      ...
      html: `
        ...
        <br/>
        <p>Connectez-vous au tableau de bord admin pour approuver ou rejeter ce compte.</p>
        <a href="https://presta-connect.vercel.app/admin-ambassadeur">Voir le tableau de bord</a>
      `,
    });
