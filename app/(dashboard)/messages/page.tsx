...
const features = [
  { icon: Zap, title: 'Connexion instantanée', description: "Dès qu'un chantier est accepté, WhatsApp s'ouvre." },
  { icon: Smartphone, title: 'Sur votre téléphone', description: "Discutez depuis votre messagerie habituelle." },
  { icon: Shield, title: 'Contacts centralisés', description: "Retrouvez vos clients dans Dashboard > Mes chantiers." },
];
...
function buildWhatsAppLink(phone: string, clientName: string, chantierTitle: string): string {
  ...
  const message = encodeURIComponent(
    `Bonjour ${clientName}, je suis votre artisan PrestaConnect concernant le chantier "${chantierTitle}". Comment puis-je vous aider ?`
  );
...
export default function MessagesPage() {
    ...
    async function loadChantiers() {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          ...
          title,
          status,
          profiles!chantiers_client_id_fkey (
            full_name,
            telephone,
            phone
          )
  ...
  return (
    <div
      className="flex items-center justify-center p-4 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50"
    ...
    >
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 mb-6 text-center">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-4">Vos discussions en temps réel</h1>
              ...
              {chantiers.map((chantier) => (
                ...
                >
                  <div>
                    ...
                    href={buildWhatsAppLink(chantier.client_phone, chantier.client_name, chantier.title)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white font-semibold text-xs px-3 py-2 rounded-xl transition-colors"
                  >
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-6 flex items-center justify-between">
          <div>
            ...
            className="flex items-center gap-2 bg-white text-blue-600 font-semibold text-sm px-4 py-2.5 rounded-xl"
          >
