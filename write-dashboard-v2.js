...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      let q = supabase.from('profiles').select('id, full_name, metier_type, metier, ville, note, diplome_verifie, avatar_url, telephone, phone').in('role', ['artisan', 'prestataire']).eq('status', 'valide');
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-8">
      ...
      <div className="flex items-center gap-4">
        <div className="w-14 h-14 bg-blue-100 rounded-2xl flex items-center justify-center overflow-hidden flex-shrink-0">
          {profile?.avatar_url ? <img src={profile.avatar_url} className="w-14 h-14 object-cover" alt="" /> : <User size={24} className="text-blue-600" />}
        </div>
        <div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-gray-900">Prestataires disponibles{ville ? ' à ' + ville : ''}</h2>
        </div>
